...
export interface FfttClientConfig {
  ...
  /** Total number of attempts per request, the first one included. */
  attempts?: number | undefined
  /** Delay before the second attempt, doubled after each new failure. */
...
const createRequestParameters = (
  config: FfttClientConfig,
  date = new Date()
): FfttRequestParameters => {
...
/**
 * The FFTT reports a functional error either as a bare `<erreur>` text node or
 * as a structured one, so the search cannot be limited to object nodes.
 */
const findErrorMessage = (value: unknown): string | undefined => {
  if (Array.isArray(value)) {
  ...
  for (const [key, child] of Object.entries(value as XmlNode)) {
    ...
    const found = findErrorMessage(child)
    if (found !== undefined) {
      return found
    }
  }
...
const text = (node: XmlNode, field: string): string | undefined => {
  ...
  return typeof scalarValue === 'string' && scalarValue.length > 0
    ? scalarValue
    : undefined
}
...
const mapClub = (node: XmlNode): FfttClub => ({
  ...
  ...(text(node, 'validation') === undefined
    ? {}
    : { validationDate: text(node, 'validation') }),
})

const mapClubDetails = (node: XmlNode): FfttClubDetails => ({
  externalId: requiredText(node, 'idclub'),
  number: requiredText(node, 'numero'),
  ...(text(node, 'nomsalle') === undefined
    ? {}
    : { roomName: text(node, 'nomsalle') }),
  ...(text(node, 'adressesalle1') === undefined
    ? {}
    : { address1: text(node, 'adressesalle1') }),
  ...(text(node, 'adressesalle2') === undefined
    ? {}
    : { address2: text(node, 'adressesalle2') }),
  ...(text(node, 'adressesalle3') === undefined
    ? {}
    : { address3: text(node, 'adressesalle3') }),
  ...(text(node, 'codepsalle') === undefined
    ? {}
    : { postalCode: text(node, 'codepsalle') }),
  ...(text(node, 'villesalle') === undefined
    ? {}
    : { city: text(node, 'villesalle') }),
  ...(text(node, 'web') === undefined ? {} : { website: text(node, 'web') }),
  ...(numberValue(node, 'latitude') === undefined
    ? {}
    : { latitude: numberValue(node, 'latitude') }),
  ...(numberValue(node, 'longitude') === undefined
    ? {}
    : { longitude: numberValue(node, 'longitude') }),
})
...
const mapOrganization = (node: XmlNode): FfttOrganization => ({
  ...
  ...(text(node, 'idpere') === undefined
    ? {}
    : { parentExternalId: text(node, 'idpere') }),
})
...
const mapTeam = (node: XmlNode): FfttTeam => ({
  ...
  ...(text(node, 'liendivision') === undefined
    ? {}
    : { divisionLink: text(node, 'liendivision') }),
  ...(text(node, 'idepr') === undefined
    ? {}
    : { eventExternalId: text(node, 'idepr') }),
  ...(text(node, 'libepr') === undefined
    ? {}
    : { eventLabel: text(node, 'libepr') }),
})
...
export const linkParameter = (
  ...
  const value = new URLSearchParams(link.replace(/^.*\?/, '')).get(name)
  return value === null || value.length === 0 ? undefined : value
}

const mapPool = (node: XmlNode, divisionExternalId: string): FfttPool => {
  const link = requiredText(node, 'lien')

  return {
    externalId: linkParameter(link, 'cx_poule') ?? link,
    divisionExternalId,
    label: requiredText(node, 'libelle'),
    link,
  }
...
const extractChampionshipDayNumber = (
...
const mapEncounter = (node: XmlNode): FfttEncounter => ({
  ...
  ...(text(node, 'dateprevue') === undefined
    ? {}
    : { plannedDate: text(node, 'dateprevue') }),
  ...(text(node, 'datereelle') === undefined
    ? {}
    : { actualDate: text(node, 'datereelle') }),
  ...(text(node, 'lien') === undefined
    ? {}
    : { detailsLink: text(node, 'lien') }),
  ...(text(node, 'ncluba') === undefined
    ? {}
    : { homeClubNumber: text(node, 'ncluba') }),
  ...(text(node, 'nclubb') === undefined
    ? {}
    : { awayClubNumber: text(node, 'nclubb') }),
  ...(text(node, 'poule') === undefined
    ? {}
    : { poolExternalId: text(node, 'poule') }),
  ...(extractChampionshipDayNumber(text(node, 'libelle')) === undefined
    ? {}
    : {
        championshipDayNumber: extractChampionshipDayNumber(
          text(node, 'libelle')
        ),
      }),
...
const mapRanking = (
  ...
  ...(text(node, 'idequipe') === undefined
    ? {}
    : { teamExternalId: text(node, 'idequipe') }),
  ...(text(node, 'idclub') === undefined
    ? {}
    : { clubExternalId: text(node, 'idclub') }),
...
const mapLicense = (node: XmlNode): FfttLicense => ({
  ...
  ...(text(node, 'sexe') === undefined
    ? {}
    : { gender: text(node, 'sexe') as 'M' | 'F' }),
  ...(text(node, 'type') === undefined
    ? {}
    : { type: text(node, 'type') as 'T' | 'P' }),
  ...(text(node, 'certif') === undefined
    ? {}
    : { medicalCertificate: text(node, 'certif') }),
  ...(text(node, 'validation') === undefined
    ? {}
    : { validationDate: text(node, 'validation') }),
  ...(text(node, 'echelon') === undefined
    ? {}
    : { echelon: text(node, 'echelon') }),
  ...(numberValue(node, 'place') === undefined
    ? {}
    : { nationalRank: numberValue(node, 'place') }),
  ...(numberValue(node, 'point') === undefined
    ? {}
    : { points: numberValue(node, 'point') }),
  ...(text(node, 'cat') === undefined
    ? {}
    : { ageCategory: text(node, 'cat') }),
})

const mapLicenseDetails = (node: XmlNode): FfttLicenseDetails => ({
  ...mapLicense(node),
  ...(numberValue(node, 'pointm') === undefined
    ? {}
    : { monthlyPoints: numberValue(node, 'pointm') }),
  ...(numberValue(node, 'apointm') === undefined
    ? {}
    : { previousMonthlyPoints: numberValue(node, 'apointm') }),
  ...(numberValue(node, 'initm') === undefined
    ? {}
    : { initialPoints: numberValue(node, 'initm') }),
  ...(text(node, 'mutation') === undefined
    ? {}
    : { mutationDate: text(node, 'mutation') }),
  ...(text(node, 'natio') === undefined
    ? {}
    : { nationality: text(node, 'natio') }),
  ...(text(node, 'arb') === undefined
    ? {}
    : { refereeGrade: text(node, 'arb') }),
  ...(text(node, 'ja') === undefined ? {} : { umpireGrade: text(node, 'ja') }),
  ...(text(node, 'tech') === undefined
    ? {}
    : { technicianGrade: text(node, 'tech') }),
  ...(text(node, 'naissance') === undefined
    ? {}
    : { birthDate: text(node, 'naissance') }),
})
...
const mapPlayerSummary = (node: XmlNode): FfttPlayerSummary => ({
  ...
  ...(text(node, 'clast') === undefined
    ? {}
    : { ranking: text(node, 'clast') }),
})
...
export const createFfttClient = (config: FfttClientConfig) => {
  ...
  const sendOnce = async (
    ...
    // The signature embeds a timestamp, so it is rebuilt for every attempt.
  ...
  const nodes = async (
    ...
    node: string,
    params: Record<string, string>
  ): Promise<XmlNode[]> => findNodes(await request(script, params), node)

  return {
    async initialize(): Promise<FfttInitialization> {
      const result = (await request('xml_initialisation.php', {})) as Record<
        string,
        unknown
      >
      ...
      return {
        ...
        ...(text(initialization, 'message') === undefined
          ? {}
          : { message: text(initialization, 'message') }),
      }
    },

    async listClubsByDepartment(department: string): Promise<FfttClub[]> {
      return (
        await nodes('xml_club_dep2.php', 'club', { dep: department })
      ).map(mapClub)
    },
    ...
    async searchClubs(search: FfttClubSearch): Promise<FfttClub[]> {
      ...
      if (entries.length !== 1) {
        throw new Error(
          'FFTT club search requires exactly one search criterion'
        )
      }
    ...
    async getClubDetails(
      clubNumber: string,
      teamId?: string
    ): Promise<FfttClubDetails> {
      const result = (
        await nodes('xml_club_detail.php', 'club', {
          club: clubNumber,
    ...
    async listOrganizations(
      type: FfttOrganizationType,
      parentId?: string
    ): Promise<FfttOrganization[]> {
      return (
    ...
    async listEvents(
      ...
      return (
        await nodes('xml_epreuve.php', 'epreuve', {
          organisme: organizationId,
          type,
        })
      ).map((node) => mapEvent(node, type))
    },
    ...
    async listTeams(
      ...
      return (
        await nodes('xml_equipe.php', 'equipe', { numclu: clubNumber, type })
      ).map(mapTeam)
    },

    async listPools(divisionId: string): Promise<FfttPool[]> {
      return (
        await nodes('xml_result_equ.php', 'poule', {
          action: 'poule',
          auto: '1',
          D1: divisionId,
        })
      ).map((node) => mapPool(node, divisionId))
    },

    async listPoolRankings(
      divisionId: string,
      poolId?: string
    ): Promise<FfttTeamRanking[]> {
      ...
      if (initialRankings.length > 0 || poolId === undefined) {
        return initialRankings.map((node) =>
          mapRanking(node, poolId ?? divisionId)
        )
      }

      const alternateRankings = await nodes(
        'xml_result_equ.php',
        'classement',
        {
          action: 'classement',
    ...
    async listPoolEncounters(
      divisionId: string,
      poolId?: string
    ): Promise<FfttEncounter[]> {
      return (
    ...
    async listEncounters(poolIds: string[]): Promise<FfttEncounter[]> {
      ...
      return (
        await nodes('xml_rencontre_equ.php', 'tour', {
          poule: poolIds.join('|'),
        })
      ).map(mapEncounter)
    },
    ...
    async getEncounterDetails(
      query: FfttEncounterDetailsQuery
    ): Promise<FfttEncounterDetails> {
      ...
      return {
        ...
        ...(numberValue(resultNode, 'resa') === undefined
          ? {}
          : { homeScore: numberValue(resultNode, 'resa') }),
        ...(numberValue(resultNode, 'resb') === undefined
          ? {}
        ...
        games: findNodes(result, 'partie').map((node) => ({
          ...
          ...(numberValue(node, 'scoreb') === undefined
            ? {}
            : { awayScore: numberValue(node, 'scoreb') }),
          ...(text(node, 'detail') === undefined
            ? {}
            : { setDetails: text(node, 'detail') }),
        })),
      }
    },
    ...
    async listPlayersByClub(clubNumber: string): Promise<FfttPlayerSummary[]> {
      ...
      return (
        await nodes('xml_liste_joueur_o.php', 'joueur', { club: clubNumber })
    ...
    async listLicensesByClub(clubNumber: string): Promise<FfttLicense[]> {
      ...
      return (
        await nodes('xml_licence_b.php', 'licence', { club: clubNumber })
      ).map(mapLicense)
    },

    async getLicense(licenseNumber: string): Promise<FfttLicenseDetails> {
      const result = (
        await nodes('xml_licence_b.php', 'licence', { licence: licenseNumber })
      )[0]
      if (result === undefined) {
